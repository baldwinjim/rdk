...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    if "RequiredTags" in valid_rule_parameters:
        for item in valid_rule_parameters["RequiredTags"].split(","):
            if item not in tags:
                return build_evaluation_from_config_item(configuration_item,"NON_COMPLIANT", "missing tags")

    for (k,v) in tags.items():
        if not v:
            return build_evaluation_from_config_item(configuration_item,"NON_COMPLIANT", "all tags present but not all populated")
...
def evaluate_parameters(rule_parameters):
    ...
    """
    if 'RequiredTags' in rule_parameters:
        
        if not rule_parameters["RequiredTags"]:
            raise ValueError("RequiredTags list is empty") 
        
        for item in rule_parameters['RequiredTags'].split(","):
            if not item.isalnum():
